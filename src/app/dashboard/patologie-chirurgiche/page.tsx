"use client"

import Link from "next/link"
import { useEffect, useMemo, useState } from "react"
import { BookOpenText, FileText, PlusCircle, Stethoscope, CalendarClock } from "lucide-react"
import { collection, onSnapshot, query, where } from "firebase/firestore"
import type { Timestamp } from "firebase/firestore"

import { DashboardSection } from "@/components/dashboard/section-shell"
import { PaginationControls } from "@/components/dashboard/pagination-controls"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Skeleton } from "@/components/ui/skeleton"
import { useAuth } from "@/contexts/auth-context"
import { db } from "@/lib/firebase"
import type { SurgicalPathology, SurgicalPathologyNote } from "@/types/patologie"
import { usePagination } from "@/hooks/use-pagination"

type TimestampLike = Timestamp | Date | { toDate?: () => Date } | null | undefined

const formatTimestamp = (timestamp?: TimestampLike) => {
  if (!timestamp) return "di recente"

  const date = (() => {
    if (timestamp instanceof Date) {
      return timestamp
    }

    const withToDate = timestamp as Timestamp | { toDate?: () => Date }

    if (typeof withToDate?.toDate === "function") {
      try {
        return withToDate.toDate()
      } catch (error) {
        console.error("Errore nel formattare la data della patologia:", error)
        return null
      }
    }

    return null
  })()

  if (!date) return "di recente"

  try {
    return new Intl.DateTimeFormat("it-IT", {
      day: "2-digit",
      month: "long",
      year: "numeric",
    }).format(date)
  } catch (error) {
    console.error("Errore nel formattare la data della patologia:", error)
    return "di recente"
  }
}

function PathologySkeleton() {
  return (
    <Card className="border-blue-200/70 bg-white/70 shadow-sm shadow-blue-100/60 backdrop-blur dark:border-blue-900/60 dark:bg-slate-950/70 dark:shadow-blue-950/40">
      <CardHeader className="space-y-3">
        <Skeleton className="h-6 w-3/4 rounded-full" />
        <Skeleton className="h-4 w-1/4 rounded-full" />
      </CardHeader>
      <CardContent className="space-y-2">
        <Skeleton className="h-3 w-full rounded-full" />
        <Skeleton className="h-3 w-5/6 rounded-full" />
      </CardContent>
    </Card>
  )
}

const getNotePreview = (note?: SurgicalPathologyNote | null) => {
  const content = note?.contenuto?.trim()
  if (!content) {
    return "Nessuna nota registrata."
  }

  return content.length > 160 ? `${content.slice(0, 157)}...` : content
}

export default function PatologieChirurgichePage() {
  const { user } = useAuth()
  const [pathologies, setPathologies] = useState<SurgicalPathology[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  const PATHOLOGIES_PER_PAGE = 6

  useEffect(() => {
    if (!user) {
      setPathologies([])
      setLoading(false)
      return
    }

    const pathologiesQuery = query(
      collection(db, "surgicalPathologies"),
      where("userId", "==", user.uid)
    )

    const unsubscribe = onSnapshot(
      pathologiesQuery,
      (snapshot) => {
        const docs: SurgicalPathology[] = snapshot.docs.map((doc) => {
          const data = doc.data() as Omit<SurgicalPathology, "id">
          return {
            id: doc.id,
            ...data,
            notesCount: typeof data.notesCount === "number" ? data.notesCount : 0,
            lastNote:
              data.lastNote && typeof data.lastNote === "object"
                ? {
                    id: String(data.lastNote.id ?? ""),
                    titolo: String(data.lastNote.titolo ?? ""),
                    contenuto: String(data.lastNote.contenuto ?? ""),
                    createdAt: data.lastNote.createdAt ?? null,
                    updatedAt: data.lastNote.updatedAt ?? null,
                  }
                : null,
          } as SurgicalPathology
        })

        setPathologies(docs)
        setLoading(false)
        setError(null)
      },
      (err) => {
        console.error("Errore nel recupero delle patologie:", err)
        setError("Impossibile recuperare le patologie al momento.")
        setLoading(false)
      }
    )

    return () => unsubscribe()
  }, [user])

  const sortedPathologies = useMemo(() => {
    return [...pathologies].sort((a, b) => {
      const aName = (a.patologia ?? "").toLowerCase()
      const bName = (b.patologia ?? "").toLowerCase()
      return aName.localeCompare(bName)
    })
  }, [pathologies])

  const {
    currentPage,
    totalPages,
    pageItems: paginatedPathologies,
    goToPrevious,
    goToNext,
    canGoPrevious,
    canGoNext,
  } = usePagination(sortedPathologies, PATHOLOGIES_PER_PAGE)

  const totalPathologies = sortedPathologies.length

  return (
    <DashboardSection
      title="Patologie chirurgiche"
      description="Visualizza le tue schede patologia, le note cliniche e i percorsi decisionali."
      actions={
        <Button
          asChild
          className="bg-linear-to-r from-sky-500 via-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/40 hover:from-sky-600 hover:via-blue-700 hover:to-indigo-700"
        >
          <Link href="/dashboard/patologie-chirurgiche/nuova" className="flex items-center gap-2">
            Nuova patologia
            <PlusCircle className="h-4 w-4" />
          </Link>
        </Button>
      }
    >
      <div className="space-y-6">
        {loading ? (
          <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-2">
            {Array.from({ length: PATHOLOGIES_PER_PAGE }).map((_, index) => (
              <PathologySkeleton key={index} />
            ))}
          </div>
        ) : null}

        {!loading && error ? (
          <Card className="border-rose-200/70 bg-rose-50/70 shadow-sm shadow-rose-100/60 dark:border-rose-900/60 dark:bg-rose-950/70 dark:shadow-rose-950/40">
            <CardHeader>
              <CardTitle className="text-rose-700 dark:text-rose-200">Si è verificato un errore</CardTitle>
              <CardDescription>{error}</CardDescription>
            </CardHeader>
          </Card>
        ) : null}

        {!loading && !error && totalPathologies === 0 ? (
          <Card className="border-blue-200/70 bg-white/70 shadow-sm shadow-blue-100/60 backdrop-blur dark:border-blue-900/60 dark:bg-slate-950/70 dark:shadow-blue-950/40">
            <CardHeader className="space-y-3 text-center">
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-linear-to-r from-sky-400 via-blue-500 to-indigo-500 text-white shadow-sm shadow-blue-500/40">
                <Stethoscope className="h-6 w-6" />
              </div>
              <CardTitle className="text-blue-800 dark:text-blue-200">Nessuna patologia registrata</CardTitle>
              <CardDescription>
                Crea la tua prima scheda patologia per raccogliere note diagnostiche e terapeutiche.
              </CardDescription>
            </CardHeader>
          </Card>
        ) : null}

        {!loading && !error && totalPathologies > 0 ? (
          <div className="space-y-4">
            <div className="md:max-h-[65vh] md:overflow-y-auto md:pr-2">
              <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-2">
                {paginatedPathologies.map((pathology) => {
                  const notesCount = typeof pathology.notesCount === "number" ? pathology.notesCount : 0
                  const primaryNote = pathology.lastNote

                  return (
                    <Card
                      key={pathology.id}
                      className="group border-blue-200/70 bg-white/80 transition hover:border-blue-300 hover:shadow-lg hover:shadow-blue-200/50 dark:border-blue-900/60 dark:bg-slate-950/80 dark:hover:border-blue-700 dark:hover:shadow-blue-900/50"
                    >
                      <Link href={`/dashboard/patologie-chirurgiche/${pathology.id}`} className="flex h-full flex-col">
                        <CardHeader className="space-y-1 pb-0">
                          <CardTitle className="flex items-center justify-between text-lg font-semibold text-slate-900 transition group-hover:text-blue-700 dark:text-slate-100 dark:group-hover:text-blue-200">
                            {pathology.patologia}
                            <BookOpenText className="h-5 w-5 text-blue-500 transition group-hover:text-indigo-500" />
                          </CardTitle>
                          <CardDescription className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                            <FileText className="h-4 w-4 text-blue-500" />
                            {notesCount === 1
                              ? "1 nota clinica disponibile"
                              : `${notesCount} note cliniche disponibili`}
                          </CardDescription>
                        </CardHeader>
                        <CardContent className="pt-5">
                          <div className="space-y-2 text-sm text-slate-500 dark:text-slate-400">
                            <p className="font-medium text-slate-700 dark:text-slate-200">
                              {primaryNote?.titolo?.trim() || "Nota clinica"}
                            </p>
                            <p>{getNotePreview(primaryNote)}</p>
                            <div className="flex items-center gap-2 text-xs text-slate-400 dark:text-slate-500">
                              <CalendarClock className="h-4 w-4 text-blue-500" />
                              Aggiornato il {formatTimestamp(pathology.updatedAt ?? pathology.createdAt)}
                            </div>
                          </div>
                        </CardContent>
                      </Link>
                    </Card>
                  )
                })}
              </div>
            </div>
            {totalPages > 1 ? (
              <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-between">
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  {totalPathologies} {totalPathologies === 1 ? "patologia" : "patologie"} totali - Pagina {currentPage} di{" "}
                  {totalPages}
                </p>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className="border-blue-200/70 text-blue-700 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-900 dark:border-blue-900/60 dark:text-blue-200 dark:hover:border-blue-700 dark:hover:bg-slate-900"
                    onClick={() => setCurrentPage((page) => Math.max(1, page - 1))}
                    disabled={currentPage === 1}
                  >
                    Precedente
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                    className="border-blue-200/70 text-blue-700 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-900 dark:border-blue-900/60 dark:text-blue-200 dark:hover:border-blue-700 dark:hover:bg-slate-900"
                    onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))}
                    disabled={currentPage === totalPages}
                  >
                    Successiva
                  </Button>
                </div>
              </div>
            ) : null}
          </div>
        ) : null}
      </div>
    </DashboardSection>
  )
}
